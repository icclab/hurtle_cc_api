...
class TestOpenShiftAdapter(unittest.TestCase):
    ...
    def test_create_app_for_success(self):
        ...
        reqs = [{"method": 'POST',
                 ...
                 "data": json.dumps(self.template_gen.get_serv()),
                 "return_code": 201,
                 },
                {"method": 'POST',
                 "url": self.URI + '/oapi/v1/namespaces/' + NAMESPACE + '/deploymentconfigs',
                 "data": json.dumps(self.template_gen.get_dc()),
                 "return_code": 201,
                 },
                {"method": 'POST',
                 "url": self.URI + '/oapi/v1/namespaces/' + NAMESPACE + '/routes',
                 "data": json.dumps(self.template_gen.get_route('.apps.ops3.cloudcomplab.ch')),
                 "return_code": 201,
                 }
                ]
    ...
    def test_get_route(self):
        suffix = 'foo'
        namespace = 'mcn'
        name = self.template_gen.name
        hostname = name + '.' + namespace + suffix

        route = self.template_gen.get_route(suffix=suffix)
        self.assertIsInstance(route, dict)
        self.assertEqual(route['metadata']['name'], name)
        self.assertEqual(route['metadata']['labels']['name'], name)
        self.assertEqual(route['spec']['host'], hostname)
        self.assertEqual(route['spec']['to']['name'], name)

    def test_get_serv(self):
        name = self.template_gen.name
        service = self.template_gen.get_serv()
        self.assertEqual(service['metadata']['name'], name)
        self.assertEqual(service['metadata']['labels']['name'], name)
        self.assertEqual(service['spec']['selector']['deploymentconfig'], name)

    def test_prepare_env(self):
        input = 'foo=bar::foo2=bar2'
        expectedOutput = [{'name': 'foo', 'value': 'bar'}, {'name': 'foo2', 'value': 'bar2'}]
        actualOutput = self.template_gen.prepare_env(input)
        ...
        expectedOutput = [{'name': 'foo', 'value': 'bar'}, {'name': 'foo', 'value': 'bar'}]
        actualOutput = self.template_gen.prepare_env(input)
        self.assertEqual(expectedOutput, actualOutput)

    def test_get_dc(self):
        name = self.template_gen.name
        docker_image = self.template_gen.docker_image
        namespace = 'mcn'
        env = [{'name': 'key1', 'value': 'val1'}, {'name': 'key2', 'value': 'val2'}]
        dc = self.template_gen.get_dc()
        self.assertEqual(dc['metadata']['name'], name)
        self.assertEqual(dc['metadata']['namespace'], namespace)
        self.assertEqual(dc['metadata']['labels']['name'], name)
        self.assertEqual(dc['metadata']['labels']['deploymentconfig'], name)
        self.assertEqual(dc['spec']['selector']['deploymentconfig'], name)
        self.assertEqual(dc['spec']['template']['metadata']['labels']['deploymentconfig'], name)
